...
## 1. Excel Files
## 2. Missing Data
## 3. Homework 2, Stem-and-Leaf, Histograms, Density Review
## 4. Rug Plots
## 5. Bar Plots (continued)
## 6. Spine and Mosaic Plots
## 7. Pie Charts, Fan Plots
#################################################

## reading in .csv files, we use
## data is separated by commas (Comma Separated Values)
## read.csv()
## name,age,grade
## Billy,17,senior
## Sally,15,freshman
## fidoasjfiodsajfioeasjrieosjaifodsmaiofrwe,102,senior
## John,,sophomore


## for reading in .txt files...
## in text files, the separator can be anything (spaces, commas, colons, tabs, semi-colons)

## text file separated by tabs
## mydata<-read.table("thisfile.txt", sep = "\t", header=T)

## text file separated by ;
## Name;Age;Occupation
## Joe;22;student
## Josephine;21;student
## mydata<-read.table("thisfile.txt", sep = ";", header=T)
...
## Install only needs to happen once
## install.packages("readxl")


## "library" needs to happen every session that you
## want to use functions in the library

## change directory to some places
R_LIBS_SITE="C:\\R-4.1.3\\library"
library(readxl)


## Read in the "poverty.xls" dataset
## Data was modified from https://www2.stetson.edu/~jrasp/data.htm
## taken from the Journal of Statistics Education online data archive.
## Data includes:
##	birthrt	Live birth rate per 1000 population
##	deathrt	Death rate per 1000 population
##	infmort	Infant deaths per 1000 population under 1 year
##	lexpm	Life expectancy at births for males
##	lexpf	Life expectancy at births for females
##	gnp		Gross National Product, per capita, US$
##	region	1 = Eastern Europe, 
##			2 = South America/Mexico, 
##			3 = Western Europe/US/Canada/Australia/NewZealand/Japan, 
##			4 = Middle East
##			5 = Asia
##			6 = Africa
...
## depends on how many 'sheets' in the file, 
## can be by 'order' or by 'names'
poverty <- read_excel("poverty.xls", sheet = 1)


## You can also read in by the sheet name
...
fan.plot(allocation,labels = task, col = taskcol,ticks = 30,
	main = "Time Online")
	## ticks = 30 means 30 tick marks all around the FULL circle
	## Each tick mark represents 360/30 = 12 degrees
	## 12/30 = 40% of the time is spent on studying


##??? PRACTICE!
##	Create a pie chart showing the grade breakdown for this course
...
########## Finding and Working with Your Own Data ##########
## Some good websites:
